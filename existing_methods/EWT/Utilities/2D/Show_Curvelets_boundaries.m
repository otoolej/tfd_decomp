...
elseif option==3
    ...
    for t=1:length(Bt)-1
        for s=1:length(Bw{t+1})
            ...
            EWT_drawArcEllipse(floor(size(f,2)/2)+1,floor(size(f,1)/2)+1,a,b,Bt(t),Bt(t+1),color);
        end
    end
    ...
    for s=1:length(Bw{end})
        ...
        EWT_drawArcEllipse(floor(size(f,2)/2)+1,floor(size(f,1)/2)+1,a,b,Bt(end),Bt(1)+pi,color);
    end
end
hold off