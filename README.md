...
The repository contains equivalent Python and MATLAB implementations. The Python package
requires Python 3.11 or newer.


## Python

Install the package and its test dependencies into a uv environment:

```sh
uv pip install -e ".[dev]"
```

The main API mirrors the MATLAB wrapper while consistently using one-dimensional NumPy
arrays:
...
# Generate the same two-component example used below.
N = 400
n = np.arange(N)
Nq = N // 4
x1 = np.cos(2 * np.pi * (0.05 * n + ((0.3 - 0.05) / (2 * N)) * n**2))
x2 = np.cos(
    2 * np.pi * (0.25 * n + ((0.4 - 0.25) / (2 * N)) * n**2) + np.pi / 4
)
x1[N - Nq :] = 0
x2[:Nq] = 0
x = x1 + x2

y, components = tfd_decomposition(x, method="tvfilt", n_components=10, plot=True)
```

`y` has shape `(N,)`; `components` is a list of arrays with the same shape. Set
`n_components=None` to retain every detected track. If no tracks are detected, the function
returns an empty array and an empty list.

Parameters are mutable and retain the MATLAB names as aliases where useful:

```python
params = DecompParams(len(x), "xtfd")
params.doppler_kernel = (151, "hann")
params.lag_kernel = (51, "hamm")
params.nfreq = 32768       # params.Nfreq is an equivalent alias

y, components = tfd_decomposition(x, "xtfd", 10, params)
```

The default xTFD frequency dimension is 8192. Increasing it can improve accuracy but also
increases the memory used by the real and complex `N x nfreq` work arrays.

The Python version preserves the numerical conventions and defaults of the MATLAB
algorithms, while correcting unambiguous implementation issues: padding is applied once,
component limits are honored during TV filtering, unknown methods raise `ValueError`, plots
do not depend on the absent `set_figure` helper, and all output vectors have one orientation.
The MATLAB `low_pass_filter` option contains no filter design; enabling it in Python raises
...
Numerical parity tests use a compact fixture generated by
`tests/matlab/generate_reference_fixtures.m`. They are skipped until that script is run in a
MATLAB environment and the resulting `tests/fixtures/matlab_reference.mat` is present.
