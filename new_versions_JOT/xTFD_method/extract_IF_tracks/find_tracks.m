...
% Started: 27-08-2021
%
% last update: Time-stamp: <2021-09-14 17:23:09 (otoolej)>
%-------------------------------------------------------------------------------
function [if_law, f_scale, t_scale, if_tracks]= ...
...
if_tracks = tracks_IF_MCQ_method(tfd_use, delta_freq_samples, ...
                                 min_component_length, params.max_no_peaks);
...
for i = 1:Ntracks
    if(size(if_tracks{i}, 1) > 0)
        ...
        if( (if_length * t_scale) >= params.min_if_length )
            n = if_tracks{i}(:, 1);
            k = if_tracks{i}(:, 2);
