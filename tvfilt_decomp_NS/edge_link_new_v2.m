function [el,ei] = edge_link_new_v2(imb, im, len, bw, fw)
...
if m1>1; aa = find(m1==min(m1)); el{aa} = [el{aa}(1,1)*ones(min(m1),1) [1:min(m1)]' ; el{aa}]; end
if m2<N(1); aa = find(m2==max(m2)); el{aa} = [el{aa} ; el{aa}(end,1)*ones(N(1)-max(m2),1) [max(m2)+1:N(1)]']; end
